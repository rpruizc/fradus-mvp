# LabelLift Paper Implementation Roadmap
...
defines LabelLift as an offline label-reconstruction engine whose outputs feed
...
- Core package: keep `labellift/` as the Python estimator and artifact package.
- Pipeline: add a Python CLI/job layer that produces durable run artifacts.
- API: add a FastAPI service in `services/api/labellift_api/`.
...
- [ ] LIFT-000-a - Add architecture decision record for replacing Streamlit.
  ...
  - Add `docs/architecture.md` that states the package + pipeline + API + web UI
    boundary, the chosen stack (`labellift/`, FastAPI, React + TypeScript +
...
- [ ] LIFT-000-b - Create package boundary for product artifacts.
  - Paper source: Algorithm 1 outputs and Section 10.5 Phase 1.
  - Add `labellift/artifacts.py` with typed path helpers for demo artifacts,
    run manifests, summaries, diagnostics, corrected labels, and reports.
  - Existing `labellift/demo_artifacts.py` must use these helpers instead of
...
- [ ] LIFT-000-c - Add pipeline run summary contract.
  - Paper source: Algorithm 1 ensures pseudo-labels, point estimate, confidence
    interval.
  - Add `labellift/run_summary.py` with dataclasses or typed dictionaries for
...
- [ ] LIFT-000-f - Add React + TypeScript + Vite web app scaffold.
...
- [ ] LIFT-000-g - Rebuild the current demo dashboard in React.
...
- [ ] LIFT-000-h - Add chart components for the modern UI.
...
- [ ] LIFT-000-i - Serve the built web UI from FastAPI.
...
- [ ] LIFT-000-j - Add one-command local product launch.
  ...
  - Add a documented command, for example `uv run python -m
    services.api.labellift_api.main` or a `Makefile` target, that starts the API
...
- [ ] LIFT-000-k - Add deployment packaging for the modern app.
...
- [ ] LIFT-000-l - Remove Streamlit from the product path.
...
- [ ] LIFT-001 - Add canonical paper schema constants.
  - Paper source: Section 3.1, Section 3.2.
  - Implement `labellift/schema.py` with named column groups for `X`, `I`,
...
- [ ] LIFT-002 - Add a schema validation result object.
...
- [ ] LIFT-003 - Derive paper gate variables from raw customer fields.
...
- [ ] LIFT-004 - Add deterministic pilot fixture CSVs.
...
- [ ] LIFT-005 - Add a CSV ingestion module.
  - Paper source: Section 10.3 nuisance function estimation inputs.
  - Implement `labellift/ingestion.py` with `load_pilot_csv(path)` and
    `validate_and_prepare_pilot_data(path, training_time)`.
  - Output must be a dataframe with the canonical schema from LIFT-001.
  - Test: valid fixture returns canonical columns and stable row counts.

- [ ] LIFT-006 - Add a browser upload validation screen.
...
- [ ] LIFT-007 - Add a `validate-data` CLI entrypoint.
  - Paper source: Section 10.5 Phase 1 offline reconstruction.
...
- [ ] LIFT-008 - Document the customer CSV contract.
...
- [ ] LIFT-009 - Add temporal window splitter.
...
- [ ] LIFT-010 - Add later-matured holdout label builder.
...
- [ ] LIFT-011 - Split synthetic backtest mode from pilot backtest mode.
...
- [ ] LIFT-012 - Add pilot baseline model.
...
- [ ] LIFT-013 - Add pilot evaluation metrics.
...
- [ ] LIFT-014 - Add pilot backtest report artifact.
  - Paper source: Section 10.5 Phase 1 and Phase 2.
...
- [ ] LIFT-015 - Add app controls for temporal backtest.
...
with `str_pseudo_outcome` and `labellift_pseudo_label`.

### Tasks

- [ ] LIFT-016 - Add K-fold assignment utility.
...
- [ ] LIFT-017 - Add cross-fitted propensity estimation.
...
- [ ] LIFT-018 - Add unclipped corruption-corrected outcome.
...
- [ ] LIFT-019 - Add collapsed STR pseudo-outcome.
...
- [ ] LIFT-020 - Add plug-in fraud-rate estimate and confidence interval.
...
- [ ] LIFT-021 - Train final conditional pseudo-label model.
  ...
  - The final pseudo-label may be clipped to `[0, 1]`; the pseudo-outcome must
    remain unclipped.
  - Test: output has both columns and only `labellift_pseudo_label` is bounded.

- [ ] LIFT-022 - Add cross-fitted collapsed STR pipeline function.
...
- [ ] LIFT-023 - Update hosted demo artifacts for STR outputs.
  ...
  - Test: `labellift.demo_artifacts` writes the new files.
...
- [ ] LIFT-024 - Add issuer-level propensity summary tables.
...
- [ ] LIFT-025 - Implement EB shrinkage weight.
...
- [ ] LIFT-026 - Apply EB shrinkage to reporting propensity.
...
- [ ] LIFT-027 - Apply EB shrinkage to maturity propensity.
...
- [ ] LIFT-028 - Apply EB shrinkage to authorization propensity.
...
- [ ] LIFT-029 - Add shrinkage mode to STR pipeline.
...
- [ ] LIFT-030 - Add positivity diagnostics.
...
- [ ] LIFT-031 - Add identified-region flags.
...
- [ ] LIFT-032 - Surface shrinkage and positivity in the app.
...
- [ ] LIFT-033 - Add stage history configuration.
...
- [ ] LIFT-034 - Add post-authorization and post-reporting feature support.
...
- [ ] LIFT-035 - Implement `mu2` regression.
...
- [ ] LIFT-036 - Implement `mu1` regression.
...
- [ ] LIFT-037 - Implement `mu0` regression.
...
- [ ] LIFT-038 - Implement general sequential score.
...
- [ ] LIFT-041 - Add estimator mode selector.
...
- [ ] LIFT-042 - Add corruption-rate table schema.
...
- [ ] LIFT-043 - Implement segment-level corruption matching.
...
- [ ] LIFT-044 - Replace global-only corruption correction.
  - Paper source: Equation 21.
...
- [ ] LIFT-045 - Add corruption sensitivity grid.
...
- [ ] LIFT-046 - Add ignorability sensitivity parameters.
...
- [ ] LIFT-047 - Add covariate balance diagnostics.
...
- [ ] LIFT-050 - Add maturity-window stability diagnostic.
  - Paper source: Section 11.7.
...
- [ ] LIFT-051 - Make `Delta` first-class in schema and features.
...
- [ ] LIFT-052 - Estimate conditional maturity propensity.
...
- [ ] LIFT-053 - Estimate network maturity curve.
...
- [ ] LIFT-054 - Add exponential maturity fit.
...
- [ ] LIFT-057 - Compute STR optimal delay.
...
- [ ] LIFT-058 - Add naive delay comparison.
...
- [ ] LIFT-059 - Add optimal-delay app section.
...
- [ ] LIFT-061 - Add estimator model config object.
...
- [ ] LIFT-062 - Add probability calibration option.
...
- [ ] LIFT-063 - Add model-performance diagnostics.
...
- [ ] LIFT-064 - Add model artifact serialization.
...
- [ ] LIFT-065 - Add model registry UI controls.
...
- [ ] LIFT-066 - Add run manifest format.
...
- [ ] LIFT-067 - Add one-command offline reconstruction CLI.
  - Paper source: Section 10.5 Phase 1 and Algorithm 1.
  - Implement `python -m labellift.run_pipeline --input ... --output ...`.
...
- [ ] LIFT-070 - Add downloadable artifacts in browser app.
...
- [ ] LIFT-071 - Add local run storage policy.
...
- [ ] LIFT-072 - Add hosted demo/pilot mode separation.
...
- [ ] LIFT-073 - Add run status and failure states.
  - Paper source: Section 10.5 operational architecture.
...
- [ ] LIFT-074 - Update README and customer instructions.
...
- [ ] LIFT-075 - Add non-monotone auxiliary-label support.
...
- [ ] LIFT-076 - Add time-varying nuisance warning.
...
- [ ] LIFT-077 - Add composite actor metadata.
...
- [ ] LIFT-078 - Add first-party/scam exclusion controls.
...
- [ ] LIFT-079 - Add production readiness QA script.
...
- [ ] LIFT-080 - Freeze v1 paper-complete acceptance report.
